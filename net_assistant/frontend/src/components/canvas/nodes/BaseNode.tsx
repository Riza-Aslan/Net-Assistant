...
import { useHAEntity } from '@/hooks/useHA'
...
export function BaseNode({ id, data, selected, icon: typeIcon, width, height }: BaseNodeProps) {
  ...
  let statusColor = theme.colors.statusColors[data.status]
  let isOnline = data.status === 'online'

  const statusEntity = useHAEntity(data.entities?.status)
  const cpuEntity = useHAEntity(data.entities?.cpu)
  const ramEntity = useHAEntity(data.entities?.ram)
  const diskEntity = useHAEntity(data.entities?.disk)
  const batteryEntity = useHAEntity(data.entities?.battery)
  const containersEntity = useHAEntity(data.entities?.containers)

  if (statusEntity) {
    const s = String(statusEntity.state).toLowerCase()
    isOnline = ['on', 'home', 'up', 'online', 'ok', 'running', 'true', '1'].includes(s)
    statusColor = isOnline ? theme.colors.statusColors.online : theme.colors.statusColors.offline
  }
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: isOnline && selected
          ...
          : 'none',
        opacity: (data.status === 'offline' && !statusEntity) || (!isOnline && statusEntity) ? 0.55 : 1,
        minWidth: 140,
    ...
    >
      ...
      {/* HA Metrics section */}
      {(cpuEntity || ramEntity || diskEntity || batteryEntity || containersEntity) && (
        <>
          <div style={{ height: 1, background: `${colors.border}44`, margin: '0 8px' }} />
          <div className="flex flex-col gap-1 px-2.5 py-1.5" style={{ color: theme.colors.nodeSubtextColor }}>
            {cpuEntity && (
              <div className="flex gap-2 items-center text-[10px] font-mono">
                <span className="w-12 shrink-0">CPU</span>
                <span className="truncate" title={cpuEntity.state}>{cpuEntity.state} %</span>
              </div>
            )}
            {ramEntity && (
              <div className="flex gap-2 items-center text-[10px] font-mono">
                <span className="w-12 shrink-0">RAM</span>
                <span className="truncate" title={ramEntity.state}>{ramEntity.state} %</span>
              </div>
            )}
            {diskEntity && (
              <div className="flex gap-2 items-center text-[10px] font-mono">
                <span className="w-12 shrink-0">Disk</span>
                <span className="truncate" title={diskEntity.state}>{diskEntity.state} %</span>
              </div>
            )}
            {batteryEntity && (
              <div className="flex gap-2 items-center text-[10px] font-mono">
                <span className="w-12 shrink-0">Battery</span>
                <span className="truncate" title={batteryEntity.state}>{batteryEntity.state} %</span>
              </div>
            )}
            {containersEntity && (
              <div className="flex gap-2 items-center text-[10px] font-mono">
                <span className="w-12 shrink-0">Docker</span>
                <span className="truncate" title={containersEntity.state}>{containersEntity.state}</span>
              </div>
            )}
          </div>
        </>
