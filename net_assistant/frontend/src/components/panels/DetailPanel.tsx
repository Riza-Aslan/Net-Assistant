  ...
  return (
    <aside className="w-72 shrink-0 flex flex-col border-l border-border bg-[#161b22] overflow-y-auto">
      ...
      <div className="px-4 py-3 border-t border-border">
        <span className="text-xs text-muted-foreground block mb-2">Home Assistant Entities</span>
        <div className="flex flex-col gap-2">
          {(['status', 'cpu', 'ram', 'disk', 'battery', 'containers'] as const).map((k) => (
            <div key={k} className="flex flex-col gap-1">
              <label className="text-[10px] text-muted-foreground uppercase">{k}</label>
              <Input
                value={data.entities?.[k] || ''}
                onChange={(e) => {
                  snapshotHistory()
                  const val = e.target.value
                  updateNode(node.id, {
                    entities: { ...(data.entities || {}), [k]: val || undefined }
                  })
                }}
                placeholder={`sensor.${k}...`}
